...
function compareVersions( lhs, rhs, reasons ) {
	...
	if ( !lhs.java.arch ) {
		...
		nojava = true;
		if ( reasons ) {
			reasons.push( 'No valid Java installation found. You need ' + javaVersionToString( rhs.java.version ) + '. You can download from ' + javaDownloadUrl );
		}
	}
	if ( !nojava && lhs.java.arch !== rhs.java.arch ) {
		same = false;
		if ( reasons ) {
	...
	if ( !nojava && !compareJavaVersions( lhs.java.version, rhs.java.version ) ) {
		same = false;
		if ( reasons ) {
